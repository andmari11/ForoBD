...
class Usuario
{
    ...
    public function __construct($nombre, $email, $password, $rol, $imagen=null, $id=null, $salt=null)
    {
        $this->id=$id;
        $this->nombre = $nombre;
        $this->email=$email;
    ...
    public static function buscaUsuarioPorNombre($nombre){
        ...
        if($result){
            ...
            if($result->num_rows>0){
                ...
                $user= new Usuario($array['nombre'], $array['email'], ($array['password']), $array['rol'], $array['imagen'],$array['id'],$array['salt'] );
                return $user;
            }
            else{
                return NULL;
    ...
    public static function insertaUsuario($usuario, $imagen) {
        ...
        if (Usuario::buscaUsuarioPorNombre($usuario->nombre) == NULL) {
            ...
            if($imagen!=null){
                ...
                $query = sprintf("INSERT INTO `usuario` (`nombre`, `email`, `password`, `rol`, `imagen`, `salt`) VALUES( '%s', '%s', '%s', '%s', '%s', '%s')",
                ...
                $query = sprintf("INSERT INTO `usuario` (`nombre`, `email`, `password`, `rol`, `salt`) VALUES( '%s', '%s', '%s', '%s', '%s')",
                $conn->real_escape_string($usuario->nombre),
                $conn->real_escape_string($usuario->email),
                $conn->real_escape_string(self::hashPassword($usuario->password_hash, $salt)), // Utiliza el hash almacenado
                $conn->real_escape_string($usuario->rol),
    ...
    public static function actualizaUsuario($username, $email, $rol, $nombreAntiguo, $password, $imagen) {
        $app = Aplicacion::getInstance();
        $conn = $app->getConexionBd();
        ...
        if ($password != "") {
            ...
            $query = "UPDATE `usuario` SET nombre='$username', email='$email', rol='$rol', password='$password_hash', salt='$salt' WHERE nombre='$nombreAntiguo'";
        } else {
